...
class PerceptionNodeComplete(Node):
    ...
    def __init__(self):
        ...
        self.depth_sub = Subscriber(self, Image, '/camera/rgbd_camera/depth/image_raw')
        ...
        self.get_logger().info('Publishing 518-dim features for LTN on /perception/detections')
    ...
    def perception_callback(self, rgb_msg: Image, depth_msg: Image):
        ...
        try:
            ...
            # Limit to 5 objects max (GPU memory)
            boxes = results.boxes[:5]

            # ── Prepare SAM once per frame ────────────────────────
            ...
            for idx, box in enumerate(boxes):
                ...
                # SAM mask overlay
                viz[mask] = (viz[mask] * 0.5 + np.array(color) * 0.5).astype(np.uint8)
                ...
                cv2.putText(viz, label, (x1, y1 - 35),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)

                pos_text = f'({position[0]:.2f}, {position[1]:.2f}, {position[2]:.2f})m'
                cv2.putText(viz, pos_text, (x1, y1 - 15),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.45, color, 1)

                cv2.putText(viz, 'CLIP✓ 512-dim', (x1, y2 + 18),
    ...
    def project_mask_to_3d(self, mask, depth_img):
        if self.camera_matrix is None:
            # Use default intrinsics if camera_info not received yet
